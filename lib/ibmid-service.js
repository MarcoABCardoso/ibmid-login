...
class IBMidService extends AuthenticatedService {
    ...
    async login({ apikey, passcode }) {
        ...
        let userData = await this.getOwnUser({ token: data.token, skipAccountValidation: true })
    ...
    async getOwnUser({ token, skipAccountValidation }) {
        return this.iamApi.validateToken({ token })
            .then(user => {
                if (!skipAccountValidation && this.options.ALLOWED_ACCOUNTS && !this.options.ALLOWED_ACCOUNTS.includes(user.account.bss)) throw new Error('Account is not allowed')
                if (this.options.ALLOWED_USERS && !this.options.ALLOWED_USERS.reduce((acc, allowed) => acc || user.email.match(allowed), false)) throw new Error('User is not allowed')
