...
class IBMidService extends AuthenticatedService {
    ...
    async manageResource({ token, resourceID, url, method, body }) {
        return this.resourceControllerApi.manageResource({ token, resourceID, url, method, body })
    }

    proxyRequest(targetURL) {
        let self = this
        return function ({ token, url, method, headers, data }) {
            let iamOptions = { url, method, headers, data }
    ...
    async proxy({ token, resourceID, url, method, headers={}, data }) {
        ...
        return this.proxyRequest(proxyURL)({ token, url, method, headers, data })
    }
