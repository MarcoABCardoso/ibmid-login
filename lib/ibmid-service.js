...
class IBMidService extends AuthenticatedService {
    ...
    constructor(
        options,
        requestApi = axios,
        iamApi = new IAMAPI(requestApi),
        accountsApi = new AccountsAPI(requestApi),
        globalCatalogApi = new GlobalCatalogAPI(requestApi),
        resourceControllerApi = new ResourceControllerAPI(requestApi)
    ) {
        super(requestApi, iamApi)
    ...
    proxyRequest(targetURL) {
        ...
        return function ({ token, url, method, headers, params, data }) {
            ...
            return self.requestApi(iamOptions)
    ...
    async getProxyURL({ token, resourceID, url, headers }) {
        ...
        if (resourceID == 'watson_data') return constants.WATSON_DATA_URL
        if (resourceID == 'account') return constants.ACCOUNTS_URL
        if (resourceID == 'billing') return constants.BILLING_URL
        if (resourceID == 'controller') return constants.RESOURCE_CONTROLLER_URL
        if (resourceID == 'catalog') return constants.GLOBAL_CATALOG_URL
        if (resourceID == 'search') return constants.GLOBAL_SEARCH_URL
        if (resourceID == 'tagging') return constants.GLOBAL_TAGGING_URL
        ...
        if (resource.id.includes('codeengine')) {
            let namespace = resource.extensions.virtual_private_endpoints.dns_domain.split('.')[0]
            if (headers['x-ce-application'])
                return `https://${headers['x-ce-application']}.${namespace}.${resource.region_id}.codeengine.appdomain.cloud`
            else if (url.startsWith('/services') || url.includes('jobs'))
                return `https://proxy.${resource.region_id}.codeengine.cloud.ibm.com/apis/serving.knative.dev/v1/namespaces/${namespace}`
            else if (url.startsWith('/pingsources'))
                return `https://proxy.${resource.region_id}.codeengine.cloud.ibm.com/apis/sources.knative.dev/v1/namespaces/${namespace}`
            else
                return `https://proxy.${resource.region_id}.codeengine.cloud.ibm.com/api/v1/namespaces/${namespace}`
        }
        ...
        if (credentials.endpoints) {
            ...
            if (headers['x-endpoint-id']) {
                let endpoints = await this.requestApi({ url: credentials.endpoints }).then(response => response.data)
