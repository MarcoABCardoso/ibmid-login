...
class IBMidService extends AuthenticatedService {
    ...
    async login(options) {
        ...
        let accountData = await this.accountsApi.listAccounts(data.token)
    ...
    async getOwnUser(options) {
        return this.iamApi.validateToken(options.cookies.token)
            .then(user => ({ statusCode: 200, body: user }))
            .catch(() => notLoggedInResponse())
    }

    async listAccounts(options) {
        return this.accountsApi.listAccounts(options.cookies.token)
    ...
    async getProxyURL(options) {
        ...
        if (options.urlParams.resource_id == 'watson_data') return 'https://api.dataplatform.cloud.ibm.com'
        ...
        if (credentials.endpoints) {
            if (options.url === '/endpoints') return credentials.endpoints.replace('/endpoints', '')
            if (options.headers['x-endpoint-id']) {
