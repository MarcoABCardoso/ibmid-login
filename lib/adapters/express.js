...
function expressify(serviceFunction) {
    return async function (req, res, next) {
        ...
        let data = await serviceFunction(options)
            .catch(error => ({ error }))
...
function getExpressAdapter(ibmidService) {
    ...
    router.authenticate = ({ fallback_url }) => (req, res, next) => {
        return expressify(async options => {
            let userData = await ibmidService.getOwnUser(options)
            if (userData.statusCode >= 400) return fallback_url ? { headers: { location: fallback_url }, statusCode: 302, body: {} } : userData
